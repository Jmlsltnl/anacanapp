/**
 * SEED GENERATORU — statik UI açarlarının hədəf dilə tam tərcüməsi.
 *
 * İstifadə:
 *   node scripts/i18n/translate-seed.cjs <lang>            # tərcümə (resume)
 *   node scripts/i18n/translate-seed.cjs <lang> --assemble # yekun faylı yığ
 * Dəstəklənən: uz, ka (yeni dil üçün LANG_CFG-yə blok əlavə et)
 */
const fs = require('fs');
...
const LANG_CFG = {
  uz: {
    name: 'UZBEK (o\'zbek tili, LATIN script, as used in Uzbekistan)',
    style: [
    ...
    mommy: 'translate it as "Onajon"',
  },
  ka: {
    name: 'GEORGIAN (ქართული, Mkhedruli script, as used in Georgia)',
    style: [
      '4) Use the polite plural "თქვენ" form when addressing the user. Warm, natural, modern Georgian — real sentences, not word-for-word.',
      '5) Terminology: ორსულობა (pregnancy), ბავშვი/პატარა (baby), მენსტრუაცია (period), მშობიარობა (birth), ჯანმო (WHO). Emergency number is 112 (unified in Georgia).',
      '6) Keep translations similar in LENGTH to the source (UI labels must stay short). Do not transliterate brand names into Georgian.',
    ],
    mommy: 'translate it as "დედიკო"',
  },
};
const CFG = LANG_CFG[LANG];
if (!CFG) { console.error('İstifadə: node translate-seed.cjs <uz|ka> [--assemble]'); process.exit(1); }
const OUT_DIR = path.join(__dirname, LANG + '.out');
...
const SYSTEM = [
  ...
  `Translate the JSON values to ${CFG.name}. Source values are Azerbaijani or English.`,
  `Rules:`,
  ...
  `3) Keep brand/product names unchanged: Anacan (app name), Premium, Dr.Anacan, Google, Apple. EXCEPTION: when "Anacan" is an affectionate address to the mother (baby speaking to mom), ${CFG.mommy}.`,
  ...CFG.style,
].join('\n');
...
function assemble() {
  ...
  fs.writeFileSync(path.join(__dirname, LANG + '.seed.json'), JSON.stringify(sorted, null, 2) + '\n');
  console.log(`${LANG}.seed.json yazıldı: ${Object.keys(sorted).length} açar (çatışmayan: ${missing})`);
  return missing;
}
