/**
 * UI açarlarının ALMAN (de) tərcüməsi — kanonik açar dəsti kk.seed.json-dur
 * (az.json + inline-default sinfi + community dinamik açarları = tam canlı kainat).
 * Mənbə prioriteti açar-açar: az.json > en.json > ru.seed > tr.seed.
 * Resume dəstəkli: de.out.json-dakı açarlar ötürülür.
 * İstifadə: node scripts/i18n/translate-ui-de.cjs [--conc 4] [--keys 35] [--limit N]
 */
const fs = require('fs');
...
const OUT_PATH = path.join(__dirname, 'de.out.json');
...
const systemFor = (srcLang) => [
  ...
  `Translate the JSON values from ${SRC_NAMES[srcLang]} to German (as used in German parenting apps).`,
  'Rules:',
  ...
  '3) Keep brand names unchanged: Anacan (app name), Premium, Dr.Anacan. EXCEPTION: "Anacan" as an affectionate address to the mother → "Mami".',
  '4) These are short UI strings (buttons, labels, toasts) — keep them concise and natural.',
  '5) Use the informal "du" form (warm, familiar tone — standard in German motherhood apps). Use "Baby" for baby, "Periode" for period. Emergency number is 112.',
...
(async () => {
  // Kanonik dəst: kk.seed açarları; mənbə: az > en > ru > tr
  const items = [];
  ...
  console.log(`UI de: kanonik ${Object.keys(KK).length} açar, hazır ${Object.keys(out).length}, tərcümə olunacaq ${todo.length} | model: ${MODEL}`);
  ...
  save();
  console.log(`✓ UI de bitdi: ${okKeys} uğurlu, ${failKeys} uğursuz | cəmi: ${Object.keys(out).length} | ${Math.round((Date.now() - t0) / 1000)}s`);
})();
