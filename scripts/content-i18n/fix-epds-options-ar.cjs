/**
 * EPDS options jsonb — text_ar əlavə edir (Ereb5.sql). fix-epds-options-de klonu.
 */
const fs = require('fs');
...
async function azureTranslate(payload) {
  const system = [
    ...
    'Translate the JSON values from Azerbaijani to Modern Standard Arabic. Address the mother in the FEMININE first-person where the options are self-statements (e.g. "نعم، معظم الوقت").',
...
(async () => {
  ...
  console.log(`✓ ${keys.length} variant ar-a tərcümə olundu`);

  const stmts = [];
  for (const row of rows) {
    const newOptions = (row.options || []).map((o) => {
      const az = o.text_az || o.text;
      return { ...o, text_ar: o.text_ar || texts.get(az) || null };
    });
  ...
  const body = [
    '-- ============================================================',
    '-- Ereb5 — EPDS cavab variantlarına text_ar əlavə olunur',
    '-- QEYD: Alman5-dən SONRA işlədin (mövcud text_ru/tr/kk/de qorunur).',
    '-- ============================================================',
    '', '',
    stmts.join('\n\n'),
    '',
  ].join('\n');
  const outPath = path.join(__dirname, '..', '..', 'supabase', 'ereb', 'Ereb5.sql');
  fs.mkdirSync(path.dirname(outPath), { recursive: true });
  fs.writeFileSync(outPath, body, 'utf8');
  console.log(`✓ supabase/ereb/Ereb5.sql — ${rows.length} UPDATE`);
})();
