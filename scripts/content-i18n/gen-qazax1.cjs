/**
 * Qazax1.sql generatoru — kk sütunları + app_languages + community CHECK.
 * REGISTRY azure-translate.cjs-dən (regex ilə) + registry-extra.json-dan oxunur —
 * tək mənbə, drift yoxdur. İstifadə: node scripts/content-i18n/gen-qazax1.cjs
 */
const fs = require('fs');
const path = require('path');

// Core REGISTRY-ni azure-translate.cjs-dən çıxar
...
for (const table of Object.keys(REGISTRY).sort()) {
  ...
  for (const [f, type] of cols) {
    lines.push(`ALTER TABLE public.${table} ADD COLUMN IF NOT EXISTS ${f}_kk ${type};`);
    colCount++;
  }
  lines.push('');
}
...
const outPath = path.join(__dirname, '..', '..', 'supabase', 'qazax', 'Qazax1.sql');
fs.mkdirSync(path.dirname(outPath), { recursive: true });
fs.writeFileSync(outPath, lines.join('\n'), 'utf8');
console.log(`✓ supabase/qazax/Qazax1.sql — ${Object.keys(REGISTRY).length} cədvəl, ${colCount} sütun`);
