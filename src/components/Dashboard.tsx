...
const BumpDashboard = ({ onNavigateToTool }: { onNavigateToTool?: (tool: string) => void }) => {
  ...
  const getTrimesterInfo = (trimester: number) => {
    switch (trimester) {
      case 1:
        return { title: t("dashboard_1_ci_trimester_tovsiyeleri_0a4c84", '1-ci Trimester Tövsiyələri'), emoji: '🌱' };
      case 2:
        return { title: t("dashboard_2_ci_trimester_tovsiyeleri_16e100", '2-ci Trimester Tövsiyələri'), emoji: '🌸' };
      case 3:
      default:
        return { title: t("dashboard_3_cu_trimester_tovsiyeleri_93f711", '3-cü Trimester Tövsiyələri'), emoji: '🍼' };
    }
  };
  ...
  const addKick = async () => {
    ...
    toast({
      title: t("dashboard_tepik_qeyd_edildi_284f06", "Təpik qeyd edildi! 👶"),
  ...
  const addWater = async () => {
    ...
    toast({
      title: t("dashboard_su_elave_edildi_7b894d", "Su əlavə edildi! 💧"),
    ...
    <div className="space-y-2">
      ...
      >
        ...
        <div className="flex justify-around">
          {[
            { icon: '👀', label: t("dashboard_goz_fbc05e", 'Göz'), active: weeklyDevelopment.eyes },
            { icon: '👂', label: 'Qulaq', active: weeklyDevelopment.ears },
            { icon: '✋', label: 'Barmaq', active: weeklyDevelopment.fingers },
            { icon: '🦶', label: t("dashboard_tepik_9a873a", 'Təpik'), active: weeklyDevelopment.kicks },
            { icon: '💇', label: t("dashboard_sac_a09eaa", 'Saç'), active: weeklyDevelopment.hair },
          ].map((item, index) => (
            <motion.div 
              key={item.label}
      ...
      <div className="grid grid-cols-4 gap-1.5">
        ...
        <QuickActionButton 
          ...
          onClick={() => {
            toast({
              title: t("dashboard_vitamin_xatirlatmasi_b8a490", "Vitamin Xatırlatması 💊"),
              description: t("dashboard_prenatal_vitamininizi_gunluk_qebul_etmey_c8cba3", "Prenatal vitamininizi günlük qəbul etməyi unutmayın. Folat, D vitamini və dəmir vacibdir!"),
            });
...
const MommyDashboard = ({ onNavigateToTool }: { onNavigateToTool?: (tool: string) => void }) => {
  ...
  const toggleSleep = async () => {
    ...
    if (sleepTimer) {
      ...
      // Start sleep
      startTimer('sleep');
      toast({ title: t("dashboard_yuxu_basladi_b503f7", "Yuxu başladı! 😴"), description: t("dashboard_bitirmek_ucun_yeniden_basin_37b592", "Bitirmək üçün yenidən basın") });
    }
  };
            ...
            >
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder={t("dashboard_diger_ml_da1e7f", "Digər (ml)")}
            ...
            >
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder={t("dashboard_mes_balkabaqli_pure_27f08d", "Məs: balkabaqlı püre")}
