...
const PartnerPrivacyScreen = ({ onBack }: PartnerPrivacyScreenProps) => {
  ...
  const handleToggle = (key: keyof typeof privacySettings) => {
    ...
    toast({ title: tr("partnerprivacyscreen_ayar_yenilendi_f0f876", 'Ayar yeniləndi') });
  };

  const handleExportData = async () => {
    if (!user) return;

    toast({ title: tr("partnerprivacyscreen_melumatlar_hazirlanir_482381", 'Məlumatlar hazırlanır...'), description: tr("partnerprivacyscreen_bu_bir_nece_saniye_ceke_biler_49b373", 'Bu bir neçə saniyə çəkə bilər.') });
    ...
    try {
      ...
      toast({ title: tr("partnerprivacyscreen_melumatlar_yuklendi_f04800", 'Məlumatlar yükləndi!') });
    } catch (error: any) {
      toast({ title: tr("partnerprivacyscreen_xeta_3cdbb6", 'Xəta'), description: error.message, variant: 'destructive' });
    }
  };
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      toast({ title: 'Hesab silindi', description: tr("partnerprivacyscreen_melumatlariniz_birdefelik_silindi_8f69e7", 'Məlumatlarınız birdəfəlik silindi.') });
    } catch (error: any) {
      toast({ title: tr("partnerprivacyscreen_xeta_3cdbb6", 'Xəta'), description: error.message, variant: 'destructive' });
    } finally {
      setDeleting(false);
  ...
  {
    icon: Shield,
    title: tr("partnerprivacyscreen_analitik_melumat_paylas_7971ff", 'Analitik məlumat paylaş'),
    description: tr("partnerprivacyscreen_anonim_istifade_melumatlarini_paylasin_835e3a", 'Anonim istifadə məlumatlarını paylaşın'),
    key: 'shareAnalytics' as const
  },
  {
    icon: Lock,
    title: tr("partnerprivacyscreen_mekan_paylasimi_bf6566", 'Məkan paylaşımı'),
    description: tr("partnerprivacyscreen_heyat_yoldasiniz_mekaninizi_gore_biler_0fb295", 'Həyat yoldaşınız məkanınızı görə bilər'),
    key: 'locationSharing' as const
  },
  {
    icon: Shield,
    title: tr("partnerprivacyscreen_bildiris_sesleri_fc9269", 'Bildiriş səsləri'),
    description: tr("partnerprivacyscreen_bildiris_geldikde_ses_cixsin_a3a866", 'Bildiriş gəldikdə səs çıxsın'),
