  ...
  return (
    <div className={cn('border border-input rounded-lg overflow-hidden bg-background', className)}>
      ...
      <div className="flex flex-wrap items-center gap-0.5 p-1.5 border-b border-border bg-muted/30">
        <MenuButton onClick={() => editor.chain().focus().toggleBold().run()} active={editor.isActive('bold')} title={t("richtexteditor_qalin_c4d6bc", "Qalın")}>
          <Bold size={s} />
        </MenuButton>
        <MenuButton onClick={() => editor.chain().focus().toggleItalic().run()} active={editor.isActive('italic')} title="Kursiv">
          <Italic size={s} />
        </MenuButton>
        <MenuButton onClick={() => editor.chain().focus().toggleUnderline().run()} active={editor.isActive('underline')} title={t("richtexteditor_altixetli_c40fd6", "Altıxətli")}>
          <UnderlineIcon size={s} />
        </MenuButton>
        <MenuButton onClick={() => editor.chain().focus().toggleStrike().run()} active={editor.isActive('strike')} title={t("richtexteditor_ustuxetli_f3d1e5", "Üstüxətli")}>
          <Strikethrough size={s} />
        </MenuButton>

        <div className="w-px h-5 bg-border mx-1" />

        <MenuButton onClick={() => editor.chain().focus().toggleHeading({ level: 1 }).run()} active={editor.isActive('heading', { level: 1 })} title={t("richtexteditor_basliq_1_4e0712", "Başlıq 1")}>
          <Heading1 size={s} />
        </MenuButton>
        <MenuButton onClick={() => editor.chain().focus().toggleHeading({ level: 2 }).run()} active={editor.isActive('heading', { level: 2 })} title={t("richtexteditor_basliq_2_2e5cb2", "Başlıq 2")}>
          <Heading2 size={s} />
        </MenuButton>
        <MenuButton onClick={() => editor.chain().focus().toggleHeading({ level: 3 }).run()} active={editor.isActive('heading', { level: 3 })} title={t("richtexteditor_basliq_3_36375a", "Başlıq 3")}>
          <Heading3 size={s} />
        </MenuButton>

        <div className="w-px h-5 bg-border mx-1" />

        <MenuButton onClick={() => editor.chain().focus().toggleBulletList().run()} active={editor.isActive('bulletList')} title={t("richtexteditor_siyahi_f04aac", "Siyahı")}>
          <List size={s} />
        </MenuButton>
        <MenuButton onClick={() => editor.chain().focus().toggleOrderedList().run()} active={editor.isActive('orderedList')} title={t("richtexteditor_nomreli_siyahi_0adbc6", "Nömrəli siyahı")}>
          <ListOrdered size={s} />
        </MenuButton>
        ...
        <MenuButton onClick={() => editor.chain().focus().setHorizontalRule().run()} title={t("richtexteditor_xett_c1e83c", "Xətt")}>
          <Minus size={s} />
        </MenuButton>
        ...
        <MenuButton onClick={() => editor.chain().focus().setTextAlign('right').run()} active={editor.isActive({ textAlign: 'right' })} title={t("richtexteditor_saga_804f56", "Sağa")}>
          <AlignRight size={s} />
        </MenuButton>
        ...
        <MenuButton onClick={onImageButtonClick} title={t("richtexteditor_sekil_yukle_0577d2", "Şəkil yüklə")}>
          <Upload size={s} />
        </MenuButton>
        ...
        <MenuButton onClick={() => editor.chain().focus().redo().run()} disabled={!editor.can().redo()} title={t("richtexteditor_i_reli_3bc432", "İrəli")}>
          <Redo size={s} />
        </MenuButton>
      </div>
