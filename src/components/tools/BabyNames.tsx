      ...
      <div className="a-shell">
        ...
        <header className="a-topbar">
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              <p className="a-eyebrow">{names.length} {tr("babynames_ad_count_3c7a2d", "ad")} · {language === 'en' ? '🇬🇧 English' : language === 'tr' ? '🇹🇷 Türkçe' : language === 'ru' ? '🇷🇺 Русские' : language === 'kk' ? '🇰🇿 Қазақша' : language === 'uz' ? '🇺🇿 Oʻzbekcha' : language === 'de' ? '🇩🇪 Deutsch' : language === 'ar' ? '🇸🇦 العربية' : `🇦🇿 ${tr("babynames_azerbaycan_733e93", "Az\u0259rbaycan")}`}</p>
                  ...
                  return (
                    <>
                      ...
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <span className="a-cta-badge" style={{ background: 'var(--a-chip-overlay)', color: ink, padding: '4px 10px', fontSize: 10 }}>
                          {selectedName.origin || (language === 'en' ? 'Azerbaijan' : language === 'tr' ? 'Türkçe' : language === 'ru' ? 'Русское' : language === 'kk' ? 'Әзербайжан' : language === 'uz' ? 'Oʻzbek' : language === 'de' ? 'International' : language === 'ar' ? 'دولي' : tr("babynames_azerbaycan_733e93", "Az\u0259rbaycan"))}
                        </span>
                      </div>
                    </>);
                })()}
