...
const readingTypes = [
  { id: 'fasting', label: t("bloodsugartracker_acliq_b6d5ee", 'Aclıq'), emoji: '🌅', description: t("bloodsugartracker_seher_tezden_yemekden_evvel_3f58e4", 'Səhər tezdən, yeməkdən əvvəl') },
  { id: 'before_meal', label: t("bloodsugartracker_yemekden_evvel_167a3d", 'Yeməkdən əvvəl'), emoji: '🍽️', description: t("bloodsugartracker_yemekden_30_deq_evvel_266704", 'Yeməkdən 30 dəq əvvəl') },
  { id: 'after_meal', label: t("bloodsugartracker_yemekden_sonra_56238e", 'Yeməkdən sonra'), emoji: '⏰', description: t("bloodsugartracker_yemekden_2_saat_sonra_c59165", 'Yeməkdən 2 saat sonra') },
  { id: 'bedtime', label: t("bloodsugartracker_yatmazdan_evvel_a457fa", 'Yatmazdan əvvəl'), emoji: '🌙', description: t("bloodsugartracker_gece_yatmaq_ucun_34ca83", 'Gecə yatmaq üçün') },
  { id: 'random', label: t("bloodsugartracker_tesadufi_98c20c", 'Təsadüfi'), emoji: '📊', description: t("bloodsugartracker_i_stenilen_vaxt_ec15be", 'İstənilən vaxt') },
];

const mealContexts = [
  { id: 'breakfast', label: t("bloodsugartracker_seher_yemeyi_b82929", 'Səhər yeməyi'), emoji: '🥞' },
  { id: 'lunch', label: 'Nahar', emoji: '🍲' },
  { id: 'dinner', label: t("bloodsugartracker_sam_yemeyi_6002e9", 'Şam yeməyi'), emoji: '🍛' },
  { id: 'snack', label: t("bloodsugartracker_qelyanalti_42fb71", 'Qəlyanaltı'), emoji: '🍎' },
];
...
const getReadingStatus = (value: number, type: string) => {
  ...
  if (type === 'fasting') {
    if (value < 70) return { status: 'low', label: t("bloodsugartracker_asagi_1c27f1", 'Aşağı'), color: 'text-blue-600', bg: 'bg-blue-500/10' };
    if (value <= 95) return { status: 'normal', label: 'Normal', color: 'text-green-600', bg: 'bg-green-500/10' };
    if (value <= 125) return { status: 'elevated', label: t("bloodsugartracker_yukselmis_1fee34", 'Yüksəlmiş'), color: 'text-amber-600', bg: 'bg-amber-500/10' };
    return { status: 'high', label: t("bloodsugartracker_yuksek_492584", 'Yüksək'), color: 'text-red-600', bg: 'bg-red-500/10' };
  } else {
    if (value < 70) return { status: 'low', label: t("bloodsugartracker_asagi_1c27f1", 'Aşağı'), color: 'text-blue-600', bg: 'bg-blue-500/10' };
    if (value <= 140) return { status: 'normal', label: 'Normal', color: 'text-green-600', bg: 'bg-green-500/10' };
    if (value <= 180) return { status: 'elevated', label: t("bloodsugartracker_yukselmis_1fee34", 'Yüksəlmiş'), color: 'text-amber-600', bg: 'bg-amber-500/10' };
    return { status: 'high', label: t("bloodsugartracker_yuksek_492584", 'Yüksək'), color: 'text-red-600', bg: 'bg-red-500/10' };
  }
};
...
const BloodSugarTracker = ({ onBack }: BloodSugarTrackerProps) => {
  ...
  const addLogMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setNotes('');
      toast({ title: t("bloodsugartracker_qeyd_elave_edildi_c5012e", 'Qeyd əlavə edildi'), description: t("bloodsugartracker_qan_sekeri_seviyyesi_ugurla_qeyd_edildi_cb5736", 'Qan şəkəri səviyyəsi uğurla qeyd edildi') });
    },
    onError: () => {
      toast({ title: t("bloodsugartracker_xeta_3cdbb6", 'Xəta'), description: t("bloodsugartracker_qeyd_elave_edile_bilmedi_c1e5e5", 'Qeyd əlavə edilə bilmədi'), variant: 'destructive' });
    },
  });
          ...
          >
            ...
            <div className="mb-4">
              ...
              <input
                ...
                placeholder={t("bloodsugartracker_meselen_95_23137b", "Məsələn: 95")}
            ...
            <div className="mb-6">
              ...
              <textarea
                ...
                placeholder={t("bloodsugartracker_elave_qeydler_c55f23", "Əlavə qeydlər...")}
