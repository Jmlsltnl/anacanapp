...
const MoodDiary = forwardRef<HTMLDivElement, MoodDiaryProps>(({ onBack }, ref) => {
  ...
  const moodEmojis = useMemo(() => {
    if (!dbMoods || dbMoods.length === 0) {
      return [
        { value: 1, emoji: '😢', label: t("mooddiary_cox_pis_e041c5", 'Çox pis'), color: 'bg-red-100 border-red-300' },
        ...
        { value: 4, emoji: '🙂', label: t("mooddiary_yaxsi_9d8595", 'Yaxşı'), color: 'bg-lime-100 border-lime-300' },
        { value: 5, emoji: '😊', label: t("mooddiary_ela_720a0e", 'Əla'), color: 'bg-green-100 border-green-300' },
      ];
    }
  ...
  const symptomOptions = useMemo(() => {
    if (!dbSymptoms || dbSymptoms.length === 0) {
      return [
        { id: 'tired', label: t("mooddiary_yorgunluq_c68d62", 'Yorğunluq'), emoji: '😴' },
        { id: 'nausea', label: t("mooddiary_urekbulanma_a42830", 'Ürəkbulanma'), emoji: '🤢' },
        { id: 'headache', label: t("mooddiary_bas_agrisi_ff6f4c", 'Baş ağrısı'), emoji: '🤕' },
      ];
    }
      ...
      <div className="px-3 -mt-3">
        <div className="bg-card rounded-xl p-1 flex gap-1 shadow-lg">
          {[
            ...
            { id: 'history', label: t("mooddiary_tarixce_b09a14", 'Tarixçə'), icon: Calendar },
            ...
            >
              ...
              <div className="bg-card rounded-3xl p-6 shadow-card border border-border/50">
                ...
                <textarea
                  ...
                  placeholder={t("mooddiary_bu_gun_haqqinda_yazmaq_istedikleriniz_1e2d2d", "Bu gün haqqında yazmaq istədikləriniz...")}
