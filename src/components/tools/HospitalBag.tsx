...
const categoryConfig = {
  documents: { label: t("hospitalbag_senedler_d60b5e", 'Sənədlər'), emoji: '📄', icon: FileText, color: 'from-amber-500 to-orange-500' },
  mom: { label: t("hospitalbag_ana_ucun_8f885e", 'Ana üçün'), emoji: '👩', icon: Package, color: 'from-pink-500 to-rose-500' },
  baby: { label: t("hospitalbag_korpe_ucun_27c058", 'Körpə üçün'), emoji: '👶', icon: Baby, color: 'from-blue-500 to-cyan-500' },
};

const priorityConfig = {
  1: { label: t("hospitalbag_cox_vacib_c4e66f", 'Çox Vacib'), color: 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400', dot: '🔴' },
  2: { label: 'Orta', color: 'bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400', dot: '🟡' },
  3: { label: t("hospitalbag_i_steye_bagli_43582b", 'İstəyə bağlı'), color: 'bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400', dot: '🟢' },
};
...
const HospitalBag = forwardRef<HTMLDivElement, HospitalBagProps>(({ onBack }, ref) => {
  ...
  const categories = [
    { id: 'all', label: t("hospitalbag_hamisi_c73c4d", 'Hamısı'), emoji: '👜' },
    { id: 'documents', label: t("hospitalbag_senedler_d60b5e", 'Sənədlər'), emoji: '📄' },
    { id: 'mom', label: 'Ana', emoji: '👩' },
    { id: 'baby', label: t("hospitalbag_korpe_fa2b51", 'Körpə'), emoji: '👶' },
  ];
