    ...
    if (dbMealTypes.length > 0) {
      return dbMealTypes.map((m) => {
        ...
        if (m.meal_id === 'nursing') {
          mealName = language === 'en' ? 'Nursing' : language === 'ru' ? 'Грудное вскармливание' : language === 'tr' ? 'Emzirme' : language === 'kk' ? 'Емізу' : language === 'de' ? 'Stillen' : 'Əmizdirmə';
          mealTime = language === 'en' ? 'Anytime' : language === 'ru' ? 'В любое время' : language === 'tr' ? 'Her zaman' : language === 'kk' ? 'Кез келген уақытта' : language === 'de' ? 'Jederzeit' : 'İstənilən vaxt';
        } else if (language === 'en') {
            ...
            <div className="a-cta" style={{ marginTop: 0 }}>
              <div className="flex justify-between items-center w-full">
                ...
                <span className="a-tag" style={{ cursor: 'default' }}>
                  {mealLogs.length} {language === 'en' ? (mealLogs.length === 1 ? 'item' : 'items') : language === 'ru' ? 'продукт' : language === 'tr' ? 'ürün' : language === 'kk' ? 'өнім' : language === 'de' ? (mealLogs.length === 1 ? 'Eintrag' : 'Einträge') : 'qida'}
                </span>
              </div>
            </div>
                      ...
                      style={{ width: '100%', background: 'none', borderLeft: 'none', borderRight: 'none', borderBottom: 'none', cursor: 'pointer' }}>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="a-list-sub">
                            {mealLogs.length > 0 ?
                          `${mealLogs.length} ${language === 'en' ? (mealLogs.length === 1 ? 'item' : 'items') : language === 'ru' ? 'продукт' : language === 'tr' ? 'ürün' : language === 'kk' ? 'өнім' : language === 'de' ? (mealLogs.length === 1 ? 'Eintrag' : 'Einträge') : 'qida'} • ${mealCalories} kcal` :
                          meal.time
                          }
                          </p>
                        </div>
