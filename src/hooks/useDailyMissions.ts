...
export const useDailyMissions = () => {
  ...
  const generateDailyMissions = useCallback(() => {
    ...
    if (lastGeneratedDate === today && dailyMissions.length > 0) {
    ...
    // Pick 5-7 missions ensuring variety
    ...
    const todayMissions = [...careMissions, ...supportMissions, ...surpriseMissions].map((m, i) => ({
      ...m,
      id: `daily-${seed}-${i}`,
    }));
  ...
  }, [lastGeneratedDate, dailyMissions]);
  ...
  }, [generateDailyMissions, lastGeneratedDate]);
